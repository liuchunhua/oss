...
time_t StrGmtToLocaltime(const char *s)
{
    int i = 0;
    char week[4] =
    { };
...
//	HMAC_CTX_cleanup(&ctx);
    HMAC(EVP_sha1(), key, key_len, (const unsigned char *) string, len,
            (unsigned char *) oss_buf, &out_len);
...
static Contents *getContents(xmlNodePtr node)
{
...
static List getCommonPrefixes(xmlNodePtr node)
{
    List list = listInit();
...
static ListBucketResult *getListBucketResult(xmlNodePtr node)
{
...
void free_Contents(Contents *content)
{
...
void free_Owner(struct Owner *owner)
{
    xmlFree(owner->id);
